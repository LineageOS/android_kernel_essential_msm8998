...
static int ah1902_probe(struct platform_device *pdev)
{
	...
	err = request_irq(pdata->irq, ah1902_irq_handle,
			IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | IRQF_SHARED, DEV_NAME, pdata);
	if(err) {
...
static int __init ah1902_init(void)
{
	return platform_driver_register(&ah1902_platform_driver);
}

static void __exit ah1902_exit(void)
{
	platform_driver_unregister(&ah1902_platform_driver);
}
late_initcall(ah1902_init);
module_exit(ah1902_exit);
